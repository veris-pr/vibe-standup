...
@Test func standupComicsEndToEnd() async throws {
    ...
    #expect(segments.count >= 1, "Should have at least 1 transcription segment")
    ...
    // Each line should have speaker attribution
    ...
    #expect(html.contains("speech-bubble"))
    #expect(html.contains("Generated by Standup"))

    // Should contain at least one speaker badge
    #expect(html.contains("speaker-badge"))

    // Count panels in HTML
    let panelCount = html.components(separatedBy: "class=\"panel ").count - 1
    #expect(panelCount >= 1, "HTML should contain rendered panels")
